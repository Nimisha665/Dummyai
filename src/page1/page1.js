...
const Page1 = () => {
  ...
  const handleSubmitButtonClick = () => {
    ...
    const tableHTML = `<table><thead><tr><th>Start Date</th><th>End Date</th><th>Email</th><th>Password</th><th>Expected PDF</th><th>OTP Verification</th><th>Random OTP</th></tr></thead><tbody>${tableContent.join('')}</tbody></table>`;
      ...
      <Container component={Paper} maxWidth="100%" style={{ padding: '20px', marginTop: '20px',  background: 'linear-gradient(to bottom,#BADAF5, white)' }}>
        ...
        {selectedPDFs.map((data, index) => (
          <ul key={index}>
            ...
            <li><strong>Password:</strong> {data.password}</li> {/* Display password */}
